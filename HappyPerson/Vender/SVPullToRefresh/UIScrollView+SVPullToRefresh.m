...
#define kSVPullArrowSize    CGSizeMake(15.f,40.f)

static CGFloat const SVPullToRefreshViewHeight = 60.f;
...
- (SVPullToRefreshArrow *)arrow {
    if(!_arrow) {
		_arrow = [[SVPullToRefreshArrow alloc]initWithFrame:CGRectMake(0, self.bounds.size.height-54, kSVPullArrowSize.width, kSVPullArrowSize.height)];
...
- (void)setLastUpdatedDate:(NSDate *)newLastUpdatedDate {
    ...
    self.dateLabel.text = [NSString stringWithFormat:NSLocalizedString(@"最后更新时间: %@",), newLastUpdatedDate?[self.dateFormatter stringFromDate:newLastUpdatedDate]:NSLocalizedString(@"从未更新",)];
}

- (void)setDateFormatter:(NSDateFormatter *)newDateFormatter {
	dateFormatter = newDateFormatter;
    self.dateLabel.text = [NSString stringWithFormat:NSLocalizedString(@"最后更新时间: %@",), self.lastUpdatedDate?[newDateFormatter stringFromDate:self.lastUpdatedDate]:NSLocalizedString(@"从未更新",)];
}
