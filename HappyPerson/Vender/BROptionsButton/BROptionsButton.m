...
- (instancetype)initForTabBar:(MHTabBarView *)tabBar forItemIndex:(NSUInteger)itemIndex delegate:(id)delegate
{
    if(self = [super init]) {
        _delegate = delegate;
        _tabBar = tabBar;
...
{
    if(self.tabBar.itemCount > self.locationIndexInTabBar) {
        
       // CGPoint pointToSuperview = [self.tabBar.superview convertPoint:item.center fromView:self.tabBar];
        
       // CGRect myRect = CGRectMake(pointToSuperview.x, pointToSuperview.y, 60, 60);
        self.frame = CGRectMake(2*(ScreenWidth/5), self.tabBar.frame.origin.y+8, (ScreenWidth/5), 50);
...
{
    if([keyPath isEqualToString:@"selectedButton"]) {
    ...
    if(state == BROptionsButtonStateClosed ||
       state == BROptionsButtonStateNormal) {
        ...
        self.closedStateImage.center = CGPointMake(self.closedStateImage.center.x,
                                                   self.frame.size.height + (self.closedStateImage.frame.size.height/2)+20);
...
- (void)changeTheButtonStateAnimatedToOpen:(NSNumber*)open
{
    ...
    if([open boolValue]){
        openImgCenter.y = self.frame.size.height + self.openedStateImage.frame.size.height;
        ...
        closedImgCenter.y = self.frame.size.height + self.closedStateImage.frame.size.height;
        [self removeBlackView];
    }
    
    self.openedStateImage.center = CGPointMake(self.frame.size.width, self.openedStateImage.center.y);
    self.closedStateImage.center = CGPointMake((self.frame.size.width) , self.closedStateImage.center.y);
...
- (void)showOptions
{
    ...
    CGFloat angle = 0.0;
    CGFloat radius = 40 * (numberOfItems) ;
    ...
    for(int i = 0; i<numberOfItems; i++) {
        ...
        CGPoint buttonPoint = CGPointMake( mypoint.x + buttonX - (kDefaultButtonHeight/3)+70,
